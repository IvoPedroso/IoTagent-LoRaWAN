...
class TtnAppService extends appService.AbstractAppService {
	...
	start(callback) {
		...
		if (process.env.TEST_TO_MOSQUITTO) {
			console.info('Test Mode in Mosquitto MQTT:\nHost: ');
			const mosquittoHost = process.env.MOSQUITTO_HOST;
			console.info(mosquittoHost);
	...
	preProcessMessage(mqttTopic, message) {
		...
		if (splittedMqttTopic.length !== 5) {
			...
			if (message) {
				let deviceEui;
				if (message.end_device_ids.dev_eui) {
					deviceEui = message.end_device_ids.dev_eui;
				}
				if (dataModel === 'application_server' && message.uplink_message.decoded_payload) {
					this.messageHandler(this, deviceId, deviceEui, message.uplink_message.decoded_payload);
				} else if (message.uplink_message.frm_payload) {
					this.messageHandler(this, deviceId, deviceEui, message.uplink_message.frm_payload);
				} else {
	...
	observeDevice(devId, devEUI, deviceObject) {
		const mqttTopic = 'v3/' + this.applicationServer.username + '/devices/' + devId + '/up';
	 ...
	 */
	observeAllDevices() {
		const mqttTopic = 'v3/' + this.applicationServer.username + '/devices/+/up';
		this.mqttClient.subscribeTopic(mqttTopic);
		winston.info('Mqtt topic subscribed:%s', mqttTopic);
	}
